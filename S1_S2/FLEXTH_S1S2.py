...
for i in range(len(lyr_ids)):
    mosaic_tiles(gfm_dir, lyr_ids[i])
...
if likelihood == True:
    with rasterio.open(gfm_dir / 'UNCERTAINTY_mosaic.tif') as src:
        ...
        with rasterio.open(gfm_dir / 'Likelihood_thr.tif', "w", **out_meta) as dst:
            dst.write(likelihood_thr, 1)
    
    print('Likelihood Layer successfully thresholded.')

#%% Resampling
# S2 has highest resolution, so all layers are resampled to that

# reproject the flood raster
input_flood_delineation = gee_dir / 'flooded.tif'
with rasterio.open(input_flood_delineation) as src:
    ...
    with rasterio.open(proc_dir / output_filename, 'w', **kwargs) as dst:
...
if roi_path != None:
    ...
    with rasterio.open(proc_dir / 'flood_reproj.tif') as src:
        raster_bounds = src.bounds
        ...
        with rasterio.open(proc_dir / 'flood_S2.tif', "w", **out_meta) as dst:
            dst.write(out_image)

print('Flood Layer reprojected.')

#%%
flood_dataset = rasterio.open(proc_dir / 'flood_S2.tif')
...
reproj(gee_dir / 'fabdem.tif', proc_dir / 'dtm.tif', proj, outputBounds, continuous_input = True)
reproj(gee_dir / 'cloud_mask.tif', proc_dir / 'exclusion_S2.tif', proj, outputBounds, continuous_input = False)
reproj(gfm_dir / 'EXCLAYER_mosaic.tif', proc_dir / 'exclusion_S1.tif', proj, outputBounds, continuous_input = False)
reproj(gfm_dir / 'OBSWATER_mosaic.tif', proc_dir / 'obswater.tif', proj, outputBounds, continuous_input = False)
reproj(gfm_dir / 'Likelihood_thr.tif', proc_dir / 'flood_S1.tif', proj, outputBounds, continuous_input = False)

#%% Combine S1 and S2 Data

# Reproject S-2 Inputs to S-1 Flood Map
# use water bodies from GFM downloads
# mask S-2 flood raster with GFM water bodies

# question: merge S-1 and S-2 Floods first, as well as their exclusion masks 
# (merging must be the intersection of both)
# or propagate both floods individually and merge later

# S2 Flood: if S2flood == 1 
# S1 Flood: if S1flood == 1 (and S2exclusion == 1)

with (
    rasterio.open(proc_dir / 'exclusion_S1.tif') as s1,
    rasterio.open(proc_dir / 'exclusion_S2.tif') as s2
 ):
    flood_s1 = s1.read(1)
    flood_s2 = s2.read(1)
    flood_s2[flood_s1 == 1] = 1
    out_meta = s2.meta.copy()

    with rasterio.open(proc_dir / 'flood.tif', "w", **out_meta) as dst:
        dst.write(flood_s2, 1)
    
# Combine Exclusion masks
with (
    rasterio.open(proc_dir / 'exclusion_S1.tif') as ex1,
    rasterio.open(proc_dir / 'exclusion_S2.tif') as ex2
 ):
    exclusion_s1 = ex1.read(1)
    exclusion_s2 = ex2.read(1)
    
    exclusion_s2[(exclusion_s2 == 1) & (exclusion_s1 == 1)] = 1 # makes sense?!
    out_meta = ex2.meta.copy()
    with rasterio.open(proc_dir / 'exclusion.tif', "w", **out_meta) as dst:
        dst.write(exclusion_s2, 1)
